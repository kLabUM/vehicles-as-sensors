%% This script generates precipitation map by fusing windshield-wiper measurements to the radar
...
%weight = 1;		% weight on prior (e.g., if weight=1, it forgets about the prior measurements and only consider the new radar measurement, default: 0.9)

%vehicleData = csvread('./data/20140811_cameracombined_filtered2.csv');
% vehicleData = csvread('./data/20140811_cameracombined_filtered_aditya.csv',1,1);
deviceIdIdx = 2 - 1; % 1
tripIx = 3 - 1; % 2
latIx = 4 - 1; % 3
lonIx = 5 - 1; % 4
...
%     vehicleData(i,13) = radarTSeries{vehicleData(i,10)+1}(vehicleData(i,12),vehicleData(i,11));
% end
% end
...
% [qx,qy,qz] = drawFigure2(M,radarTSeries{1}/grandmax,dSamp);
% ++++++++++++PLOT++++++++++++
...
if ~isempty(timeHorz)
    for u1 = 1:length(timeHorz)     % for each time from the 'timeHorz'
        ...
        %% update prior when necessary
        % new prior = (previous prior)*(1-weight) + (new radar measurement)*weight
        % if weight = 1 then, Mt = M
        ...
        % compute expected radar intensity (normalized)
...
%                 nearPosUniqVal(:,2) = vehicleDataEff(find(vehicleDataEff(:,end) == nearPosUniq(j)),3);
            end
...
%             for i = 1:size(sampPos,1)
%                 particleWgt(:,i) = sampInfo' * prvWgt(:,i);
%             end
            clear M;
            M(:,1) = qx(:);
            M(:,2) = qy(:);        
